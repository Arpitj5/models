...
class BertEncoder(tf.keras.Model):
  ...
  *Note* that the network is constructed by
  ...
  def __init__(
    ...
    encoder_outputs = []
    # 768 divisors: 1, 2, 3, 4, 6, 8, 12, 16, 24, 32, 48, 64
    #num_attention_heads_by_layer = np.repeat(int(sys.argv[1]),num_layers)
    #if(int(sys.argv[3]) != 0):
    #  num_attention_heads_by_layer[int(sys.argv[2])] = int(sys.argv[3])
    ...
    for i in range(num_layers):
      ...
      layer = layers.TransformerEncoderBlock(
          #num_attention_heads=random.choice([16,12,24]),
          #num_attention_heads=num_attention_heads_by_layer[i],
          num_attention_heads=num_attention_heads,
          inner_dim=inner_dim,
          inner_activation=inner_activation,
          output_dropout=output_dropout,
