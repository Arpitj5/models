...
@keras_export("keras.layers.experimental.EinsumDense_local")
class EinsumDense_local(Layer):
  ...
  **Biased dense layer with einsums**
  ...
  >>> layer = EinsumDense_local("ab,bc->ac", output_shape=64, bias_axes="c")
  ...
  **Applying a dense layer to a sequence**
  ...
  >>> layer = EinsumDense_local("abc,cd->abd",
  ...
  **Applying a dense layer to a sequence using ellipses**
  ...
  >>> layer = EinsumDense_local("...x,xy->...y", output_shape=64, bias_axes="y")
  >>> input_tensor = tf.keras.Input(shape=[32, 128])
  >>> output_tensor = layer(input_tensor)
  >>> output_tensor
  <... shape=(None, 32, 64) dtype=...>
  ...
  def __init__(self,
               ...
               **kwargs):
    super(EinsumDense_local, self).__init__(**kwargs)
  ...
  def build(self, input_shape):
    ...
    self.kernel = self.add_weight(
        ...
        dtype=self.dtype,
        trainable=True)
    #print("XXXXXXX:",self.kernel)
    #self.kernel_weights = tf.Variable(tf.zeros(tf.TensorShape(self.kernel.get_shape())))
    if bias_shape is not None:
      ...
      self.bias = None
    super(EinsumDense_local, self).build(input_shape)
  ...
  def get_config(self):
    ...
    base_config = super(EinsumDense_local, self).get_config()
    return dict(list(base_config.items()) + list(config.items()))

  #def call(self, inputs):
  def call(self, inputs,head_num_list=None,num_heads=12):
    #kernel_weights = self.kernel
    #ret = special_math_ops.einsum(self.equation, inputs, self.kernel)
    
    #TODO: Check profiler to see if kernel_weights are getting copied from CPU to GPU
    #for index in range(1,12):
      #kernel_weights[index,:,:]
      #self.kernel_weights[index,:,:] = 0
      #if(index not in head_num_list):
    ...
    if head_num_list:
      ...
      input_weights = tf.slice(inputs,[0,0,start_idx,0],[-1,-1,head_num_list[0],-1])
      start_idx = head_num_list[0]+1
      for head_drp_idx in head_num_list[1:]:
        kernel_slice =  tf.slice(self.kernel,[start_idx,0,0],[head_drp_idx - start_idx,-1,-1])
        kernel_weights = tf.concat([kernel_weights, kernel_slice],0)
        input_slice = tf.slice(inputs,[0,0,start_idx,0],[-1,-1,head_drp_idx - start_idx,-1])
        input_weights = tf.concat([input_weights, input_slice],2)
        start_idx = head_drp_idx+1
      kernel_slice =  tf.slice(self.kernel,[start_idx,0,0],[num_heads - start_idx,-1,-1])
      kernel_weights = tf.concat([kernel_weights, kernel_slice],0)
      input_slice = tf.slice(inputs,[0,0,start_idx,0],[-1,-1,num_heads - start_idx,-1])
      input_weights = tf.concat([input_weights, input_slice],2)
    else:
      kernel_weights =  tf.slice(self.kernel,[start_idx,0,0],[num_heads,-1,-1])
      input_weights = tf.slice(inputs,[0,0,start_idx,0],[-1,-1,num_heads,-1])
      

    #kernel_weights = tf.slice(self.kernel,[0,0,0],[12,64,768])
    ...
    tf.print("Einsum:",kernel_weights.get_shape(),input_weights.get_shape(),ret.get_shape())
