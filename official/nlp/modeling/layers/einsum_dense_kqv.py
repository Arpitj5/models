...
@keras_export("keras.layers.experimental.EinsumDense_kqv")
class EinsumDense_kqv(Layer):
  ...
  **Biased dense layer with einsums**
  ...
  >>> layer = EinsumDense_kqv("ab,bc->ac", output_shape=64, bias_axes="c")
  ...
  **Applying a dense layer to a sequence**
  ...
  >>> layer = EinsumDense_kqv("abc,cd->abd",
  ...
  **Applying a dense layer to a sequence using ellipses**
  ...
  >>> layer = EinsumDense_kqv("...x,xy->...y", output_shape=64, bias_axes="y")
  >>> input_tensor = tf.keras.Input(shape=[32, 128])
  >>> output_tensor = layer(input_tensor)
  >>> output_tensor
  <... shape=(None, 32, 64) dtype=...>
  ...
  def __init__(self,
               ...
               **kwargs):
    super(EinsumDense_kqv, self).__init__(**kwargs)
  ...
  def build(self, input_shape):
    ...
    if bias_shape is not None:
      ...
      self.bias = None
    super(EinsumDense_kqv, self).build(input_shape)
  ...
  def get_config(self):
    ...
    base_config = super(EinsumDense_kqv, self).get_config()
  ...
  def call(self, inputs,head_num_list=None,num_heads=12):
    ...
    start_idx = 0
    if head_num_list:
      kernel_weights =  tf.slice(self.kernel,[0,start_idx,0],[-1,head_num_list[0],-1])
      bias_weights = tf.slice(self.bias,[start_idx,0],[head_num_list[0],-1])
      start_idx = head_num_list[0]+1
      for head_drp_idx in head_num_list[1:]:
        kernel_slice =  tf.slice(self.kernel,[0,start_idx,0],[-1,head_drp_idx - start_idx,-1])
        kernel_weights = tf.concat([kernel_weights, kernel_slice],1)
        bias_slice =  tf.slice(self.bias,[start_idx,0],[head_drp_idx - start_idx,-1])
        bias_weights = tf.concat([bias_weights, bias_slice],0)
        start_idx = head_drp_idx+1
      kernel_slice =  tf.slice(self.kernel,[0,start_idx,0],[-1,num_heads - start_idx,-1])
      kernel_weights = tf.concat([kernel_weights, kernel_slice],1)
      bias_slice =  tf.slice(self.bias,[start_idx,0],[num_heads - start_idx,-1])
      bias_weights = tf.concat([bias_weights, bias_slice],0)
    else:
      kernel_weights =  tf.slice(self.kernel,[0,start_idx,0],[-1,num_heads,-1])
      bias_weights = tf.slice(self.bias,[start_idx,0],[num_heads,-1])
      
    #tf.print(self.equation)
    #tf.print(inputs.get_shape())
    #tf.print(self.kernel.get_shape())
    #tf.print(kernel_weights.get_shape())
    #print(self.bias)
    #print(bias_weights)
    ...
    #tf.print(ret.get_shape())
    #tf.print(ret2.get_shape())
    return ret
